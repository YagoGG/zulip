...
def create_user_profile(realm, email, password, active, bot_type, full_name,
                        bot_owner, is_mirror_dummy, tos_version, timezone,
                        tutorial_status=UserProfile.TUTORIAL_WAITING,
                        enter_sends=False):
    # type: (Realm, Text, Optional[Text], bool, Optional[int], Text, Optional[UserProfile], bool, Text, Optional[Text], Optional[Text], bool) -> UserProfile
    now = timezone_now()
    email = UserManager.normalize_email(email)

    user_profile = UserProfile(email=email, is_staff=False, is_active=active,
                               full_name=full_name, last_login=now,
                               date_joined=now, realm=realm, pointer=-1,
...
def create_user(email, password, realm, full_name, active=True, bot_type=None,
                bot_owner=None, tos_version=None,
    ...
    # type: (Text, Optional[Text], Realm, Text, bool, Optional[int], Optional[UserProfile], Optional[Text], Text, Text, bool, Optional[Stream], Optional[Stream], Optional[bool], Optional[int]) -> UserProfile
    user_profile = create_user_profile(realm, email, password, active, bot_type,
                                       full_name, bot_owner, is_mirror_dummy,
                                       tos_version, timezone)
